...
func TestCacheWithClient(t *testing.T) {
	htmlcoinMockServer := NewHtmlcoinMockServer(mockJsonRPCResponse)
	URL := "http://htmlcoinuser:htmlcoinpass@127.0.0.1:6969"
	defer htmlcoinMockServer.Close()
	...
	client.URL = htmlcoinMockServer.URL
	if err != nil {
		t.Fatal(err)
	}

	var result interface{}
	t.Run("Htmlcoin RPC getblock call processed succesfully", func(t *testing.T) {
	...
	t.Run("Upon second getblock call, response is returned from cache", func(t *testing.T) {
		...
		if !strings.Contains(outputLog, "htmlcoin (CACHED) RPC response") {
			t.Errorf("\nexpected: %s\n\n, got: %s", "htmlcoin (CACHED) RPC response", outputLog)
		}
...
func NewHtmlcoinMockServer(body []byte) *httptest.Server {
